...
console.log('========== 任务3验证：一年级语文 ==========');
console.log('');

console.log('【知识点文件检查】');

const upperPath = path.join(KNOWLEDGE_DIR, 'g1_chinese_upper.json');
const lowerPath = path.join(KNOWLEDGE_DIR, 'g1_chinese_lower.json');
const reciteUpperPath = path.join(KNOWLEDGE_DIR, 'g1_chinese_recite_upper.json');
const reciteLowerPath = path.join(KNOWLEDGE_DIR, 'g1_chinese_recite_lower.json');

check(fs.existsSync(upperPath), '上册知识点文件存在');
check(fs.existsSync(lowerPath), '下册知识点文件存在');
check(fs.existsSync(reciteUpperPath), '重点背诵上册文件存在');
check(fs.existsSync(reciteLowerPath), '重点背诵下册文件存在');
...
const reciteUpperData = JSON.parse(fs.readFileSync(reciteUpperPath, 'utf-8'));
const reciteLowerData = JSON.parse(fs.readFileSync(reciteLowerPath, 'utf-8'));

check(upperData.length === 30, '上册知识点数量：' + upperData.length + '/30');
check(lowerData.length === 30, '下册知识点数量：' + lowerData.length + '/30');
check(reciteUpperData.length === 15, '重点背诵上册数量：' + reciteUpperData.length + '/15');
check(reciteLowerData.length === 15, '重点背诵下册数量：' + reciteLowerData.length + '/15');
...
check(validateIds(upperData, 'g1_chinese_', 'u'), '上册ID格式正确（g1_chinese_u001~u030）');
check(validateIds(lowerData, 'g1_chinese_', 'l'), '下册ID格式正确（g1_chinese_l001~l030）');
check(validateIds(reciteUpperData, 'g1_chinese_recite_', 'u'), '重点背诵上册ID格式正确');
check(validateIds(reciteLowerData, 'g1_chinese_recite_', 'l'), '重点背诵下册ID格式正确');
...
check(validateContent(reciteUpperData), '重点背诵上册内容完整');
check(validateContent(reciteLowerData), '重点背诵下册内容完整');
...
if (failed === 0) {
    console.log('');
    console.log('🎉 任务3验证全部通过！');
} else {
    console.log('');
    console.log('❌ 任务3验证未通过，需要修复！');
    process.exit(1);
}
