// 独立验证脚本：检查所有 g2_math_XXX_010.json 文件
const fs = require('fs');
const path = require('path');

const dir = 'd:\\obj\\学生复习\\data\\examples';
const files = fs.readdirSync(dir).filter(f => /^g2_math_\d{3}_010\.json$/.test(f)).sort();

console.log(`找到 ${files.length} 个 g2_math_XXX_010.json 文件\n`);
...
// 抽样显示前两个文件内容
console.log(`\n=== 抽样: g2_math_001_010.json ===`);
const sample1 = JSON.parse(fs.readFileSync(path.join(dir, 'g2_math_001_010.json'), 'utf-8'));
console.log(`第1题: ${sample1[0].question.substring(0, 60)}...`);
console.log(`第1答: ${sample1[0].answer.substring(0, 60)}...`);

console.log(`\n=== 抽样: g2_math_053_010.json ===`);
const sample53 = JSON.parse(fs.readFileSync(path.join(dir, 'g2_math_053_010.json'), 'utf-8'));
console.log(`第1题: ${sample53[0].question.substring(0, 60)}...`);
console.log(`第1答: ${sample53[0].answer.substring(0, 60)}...`);
