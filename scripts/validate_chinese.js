// 验证所有 g2_chinese_XXX_010.json 文件
const fs = require('fs');
const path = require('path');

const dir = 'd:\\obj\\学生复习\\data\\examples';
const files = fs.readdirSync(dir).filter(f => /^g2_chinese_[ul]\d{3}_010\.json$/.test(f)).sort();

console.log(`找到 ${files.length} 个 g2_chinese_XXX_010.json 文件\n`);
...
for (const file of files) {
  ...
  try {
    ...
    if (!Array.isArray(data)) {
      console.log(`X ${file}: 不是数组`);
      allValid = false;
      continue;
    }

    if (data.length !== 10) {
      console.log(`X ${file}: 题目数量 ${data.length} != 10`);
      allValid = false;
      continue;
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!item.id || item.id !== expectedId) {
        console.log(`X ${file} 第${i + 1}题: id不正确 (${item.id} != ${expectedId})`);
        allValid = false;
        fileHasIssue = true;
      }
      if (!item.knowledge_id) {
        console.log(`X ${file} 第${i + 1}题: 缺少 knowledge_id`);
        allValid = false;
        fileHasIssue = true;
      }
      if (!item.question || typeof item.question !== 'string' || item.question.length < 5) {
        console.log(`X ${file} 第${i + 1}题: question 缺失或过短`);
        allValid = false;
        fileHasIssue = true;
      }
      if (!item.answer || typeof item.answer !== 'string' || item.answer.length < 5) {
        console.log(`X ${file} 第${i + 1}题: answer 缺失或过短`);
        allValid = false;
        fileHasIssue = true;
      }
    ...
    console.log(`X ${file}: 解析失败 - ${e.message}`);
    allValid = false;
  }
}
...
console.log(`全部有效: ${allValid ? 'YES' : 'NO'}`);
...
console.log(`\n=== 分类统计 ===`);
console.log(`上册 (u001-u030): ${upperCount} 个文件`);
console.log(`下册 (l001-l030): ${lowerCount} 个文件`);
console.log(`上册有效文件: ${upperValid} 个`);
console.log(`下册有效文件: ${lowerValid} 个`);

// 抽样显示
console.log(`\n=== 抽样: g2_chinese_u001_010.json ===`);
try {
  const sample1 = JSON.parse(fs.readFileSync(path.join(dir, 'g2_chinese_u001_010.json'), 'utf-8'));
...
console.log(`\n=== 抽样: g2_chinese_l001_010.json ===`);
try {
  const sample2 = JSON.parse(fs.readFileSync(path.join(dir, 'g2_chinese_l001_010.json'), 'utf-8'));
...
console.log(`\n=== 抽样: g2_chinese_u030_010.json ===`);
try {
  const sample3 = JSON.parse(fs.readFileSync(path.join(dir, 'g2_chinese_u030_010.json'), 'utf-8'));
...
console.log(`\n=== 抽样: g2_chinese_l030_010.json ===`);
try {
  const sample4 = JSON.parse(fs.readFileSync(path.join(dir, 'g2_chinese_l030_010.json'), 'utf-8'));
