...
console.log('========== 任务5验证：一年级地理 ==========');
console.log('');

console.log('【知识点文件检查】');

const upperPath = path.join(KNOWLEDGE_DIR, 'g1_geography_upper.json');
const lowerPath = path.join(KNOWLEDGE_DIR, 'g1_geography_lower.json');
...
check(validateIds(upperData, 'g1_geography_', 'u'), '上册ID格式正确（g1_geography_u001~u030）');
check(validateIds(lowerData, 'g1_geography_', 'l'), '下册ID格式正确（g1_geography_l001~l030）');
...
if (failed === 0) {
    console.log('');
    console.log('🎉 任务5验证全部通过！');
} else {
    console.log('');
    console.log('❌ 任务5验证未通过，需要修复！');
    process.exit(1);
}
