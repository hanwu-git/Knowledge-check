...
// 加载上册数据文件
const upperFiles = ['chinese_upper_1.js', 'chinese_upper_2.js'];
...
// 加载下册数据文件
const lowerFiles = ['chinese_lower_1.js', 'chinese_lower_2.js'];
...
// 验证目录中所有g2_chinese_XXX_010.json文件
console.log(`\n=== 目录验证 ===`);
const dirFiles = fs.readdirSync(outputDir).filter(f => /^g2_chinese_[ul]\d{3}_010\.json$/.test(f)).sort();
console.log(`目录中 g2_chinese_XXX_010.json 格式的文件数: ${dirFiles.length}`);
