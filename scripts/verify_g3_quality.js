...
function checkDuplicateOptions(question) {
  if (!question.includes('选择') && !question.match(/[A-D]\./)) return null;
...
const subject = process.argv[2] || 'history';
const prefix = 'g3_' + subject + '_';

console.log(`正在检查初三${subject}例题质量...\n`);

const files = fs.readdirSync(EX_DIR).filter(f => f.startsWith(prefix) && f.endsWith('.json'));
...
for (const file of files) {
  try {
    ...
    for (const ex of examples) {
      ...
      if (typeof ex.question !== 'string' || typeof ex.answer !== 'string') {
        totalProblems++;
        problems.push({ file, id: ex.id, type: '字段类型错误，不是字符串' });
        continue;
      }
    ...
    totalProblems++;
    problems.push({ file, id: '-', type: 'JSON解析错误：' + e.message });
  }
}
